...
TrajectoryOperationCircle::TrajectoryOperationCircle(const std::string & name)
	:TrajectoryBase(name)
{}

unsigned int TrajectoryOperationCircle::size() const
{
	return 6;
}

const TrajectorySample & TrajectoryOperationCircle::operator()(double)
{
	return m_sample;
}

const TrajectorySample & TrajectoryOperationCircle::computeNext()
{
	...
	if (m_time < m_stime) {
		...
		// }
		double w = M_PI/20.0;
		cubic_tra(0) = m_init.translation()(0) ;
		cubic_tra(1) = m_init.translation()(1) + m_radius*sin((m_time-m_stime)*w) ;
		cubic_tra(2) = m_init.translation()(2) + m_radius*cos((m_time-m_stime)*w + M_PI) + m_radius;
...
bool TrajectoryOperationCircle::has_trajectory_ended() const
{
	return true;
}
void TrajectoryOperationCircle::setInitSample(Transform3d init_M)
{
	m_init = init_M;
}
void TrajectoryOperationCircle::setDuration(const double & duration)
{
	m_duration = duration;
}
void TrajectoryOperationCircle::setCurrentTime(const double & time)
{
	m_time = time;
}
void TrajectoryOperationCircle::setStartTime(const double & time)
{
	m_stime = time;
}

void TrajectoryOperationCircle::setReference(const Transform3d ref) {
	m_sample.resize(12, 6);
	m_sample.pos.head<3>() = ref.translation();
	typedef Eigen::Matrix<double, 9, 1> Vector9;
	m_sample.pos.tail<9>() = Eigen::Map<const Vector9>(&ref.rotation()(0), 9);
}
void TrajectoryOperationCircle::setRadius(const double radius){
	m_radius = radius;
}

void TrajectoryOperationCircle::getLastSample(TrajectorySample & sample) const
{
    sample = m_sample;
}