...
InverseDynamics::InverseDynamics(RobotModel & robot, bool verbose)
	: m_robot(robot), m_verbose(verbose), m_baseDynamics("base-dynamics", 6, robot.nv()), m_solutionDecoded(false) {
	m_t = 0.0;
	m_v = robot.nv();
	m_k = 0;
...
const HQPData & InverseDynamics::computeProblemData(double time, VectorXd q, VectorXd v, bool transition)
{
	m_t = time;
...
bool InverseDynamics::decodeSolution(const HQPOutput & sol)
{
	...
	if (m_robot.type() == 0) {
		m_dv = sol.x.head(m_v);
	//	m_tau = h_a;
	//	m_tau.noalias() += M_a*m_dv - 0.15 * m_robot.getRealJointVelocity().tail(m_v);
		m_tau = m_dv;
	...
	if (m_robot.type() == 1) {
		...
		m_tau.noalias() += m_dv;// -0.15 * m_robot.getRealJointVelocity().tail(m_v - 2);
